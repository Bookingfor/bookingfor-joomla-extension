...
$db->setQuery('SELECT id FROM #__menu WHERE link LIKE '. $db->Quote( $uri .'%' ) .' AND (language='. $db->Quote($lang) .' OR language='.$db->Quote('*').') LIMIT 1' );
...
$db->setQuery('SELECT id FROM #__menu WHERE link LIKE '. $db->Quote( $uriFav ) .' AND (language='. $db->Quote($language) .' OR language='.$db->Quote('*').') LIMIT 1' );
	...
	<!--
		...
		function openGoogleMapResource() {
			if (typeof google !== 'object' || typeof google.maps !== 'object'){
				...
				script.type = "text/javascript";
				script.src = "http://maps.google.com/maps/api/js?libraries=drawing&sensor=false&callback=handleApiReady";
