...
$bfc_display = $this->params['default_display'];
...
var bfc_display = '<?php echo $bfc_display ?>';
//-->
</script>
...
if (localStorage.getItem('displaymerchants')) {
	if (localStorage.getItem('displaymerchants') == 'list') {
...
<?php endif; ?>
				...
				if (val.RatingsContext!= null && (val.RatingsContext == '1' || val.RatingsContext == '3')){
					...
					if (val.Avg != null && val.Avg != '' ) {
						...
						jQuery("#ratingAnchorGridvalue"+val.MerchantId).html(number_format(val.Avg.Average, 1, '.', ''));
						jQuery("#ratingAnchorGridcount"+val.MerchantId).html(strRatingBased.replace("%s", val.Avg.Count));
						
					}else{
						if (val.RatingsType!= null && (val.RatingsType == '0' || val.RatingsType == '3')){
						$htmlAvg += strRatingNoResult;
						jQuery("#ratingAnchorcount"+val.MerchantId).html($htmlAvg);
						jQuery("#ratingAnchorGridcount"+val.MerchantId).html($htmlAvg);
						}
					...
					jQuery("#ratingAnchorGridvalue"+val.MerchantId).parent().hide();		
					jQuery("#ratingAnchorGridcount"+val.MerchantId).parent().hide();				
				}
		});	
