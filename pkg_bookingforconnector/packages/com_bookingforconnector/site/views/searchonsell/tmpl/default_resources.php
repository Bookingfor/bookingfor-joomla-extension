...
$zoneiddefault =  $this->params['zoneId'];
...
$ordselect = array(
	...
	JHTML::_('select.option', 'price|asc', JTEXT::_('COM_BOOKINGFORCONNECTOR_SEARCH_VIEW_ORDERBY_STAYASC') ),
	JHTML::_('select.option', 'price|desc',JTEXT::_('COM_BOOKINGFORCONNECTOR_SEARCH_VIEW_ORDERBY_STAYDESC')),
	JHTML::_('select.option', 'AddedOn|asc', JTEXT::_('COM_BOOKINGFORCONNECTOR_SEARCH_VIEW_ORDERBY_CREATEDASC') ),
	JHTML::_('select.option', 'AddedOn|desc',JTEXT::_('COM_BOOKINGFORCONNECTOR_SEARCH_VIEW_ORDERBY_CREATEDESC')),
);
...
if (isset($locationZones) && count($locationZones)>0){
	foreach ($locationZones as $lz) {
		if(strpos($lz,'|') !== false){
			$itm = explode('|',$lz);
			if($zoneiddefault != 0 && $itm[0]==$zoneiddefault){
				$locationZoneSelectedName = $itm[1];
			}
			$listlocationZones[] = JHTML::_('select.option', $itm[0],$itm[1]);

		}else{
			if($zoneiddefault != 0 && $lz==$zoneiddefault){
				$locationZoneSelectedName = $lz;
			}
...
?>
	<h3 class="mod_bookingforconnector_merchantdetails-menuTitle"><a href="<?php echo $route?>" class="item"><span class="fn org"><?php echo ""?></span></a></h3>
<?php
}
...
$contractType = JTEXT::_('COM_BOOKINGFORCONNECTOR_VIEWS_ONSELLUNIT_CONTRACTTYPE');
if ($this->params['contractTypeId']=="1"){
	$contractType = JTEXT::_('COM_BOOKINGFORCONNECTOR_VIEWS_ONSELLUNIT_CONTRACTTYPE1');
}


$pathway->addItem(
	$contractType,
	"javascript:setOnlyContract()"
);

$cityId = $this->params['cityId'];
if(isset($cityId ) && $cityId >0){
	$location = BFCHelper::getLocationById($cityId);
	$pathway->addItem(
		$location->Name,
		"javascript:resetLocation()"
	);

}

if(!empty($locationZoneSelectedName )){
	$pathway->addItem(
		$locationZoneSelectedName,
		""
	);
...
<!--
function setOnlyContract(){
	jQuery("#searchformonsellunit #zoneId").val(0);
//	jQuery("#searchformonsellunit #unitCategoryId").val(0);
	jQuery("#searchformonsellunit").submit();
}
function resetLocation(){
	jQuery("#bookingforsearchForm #zoneId").val(0);
	jQuery("#bookingforsearchForm").submit();
}
...
<div class="com_bookingforconnector-search-menu">
	<form action="<?php echo htmlspecialchars(JFactory::getURI()->toString()); ?>" method="post" name="bookingforsearchForm" id="filterForm">
	...
	<div class="com_bookingforconnector-results-sort">
		...
		<span class="com_bookingforconnector-sort-item" rel="price|asc" <?php echo $hidesort ?>><?php echo JTEXT::_('COM_BOOKINGFORCONNECTOR_SEARCH_VIEW_ORDERBY_STAYASC'); ?></span>
		<span class="com_bookingforconnector-sort-item" rel="AddedOn|asc" <?php echo $hidesort ?>><?php echo JTEXT::_('COM_BOOKINGFORCONNECTOR_SEARCH_VIEW_ORDERBY_CREATEDASC'); ?></span>
	</div>
	...
	<div class="<?php echo COM_BOOKINGFORCONNECTOR_BOOTSTRAP_COL ?>12 com_bookingforconnector-item-col" >
		<div class="com_bookingforconnector-search-resource com_bookingforconnector-item <?php echo COM_BOOKINGFORCONNECTOR_BOOTSTRAP_ROW ?>">
			...
			<div class="com_bookingforconnector-offers-item-primary com_bookingforconnector-item-primary <?php echo COM_BOOKINGFORCONNECTOR_BOOTSTRAP_COL ?>6">
				<div class="com_bookingforconnector-offers-item-primary-name com_bookingforconnector-item-primary-name">
					<div class="com_bookingforconnector-offers-item-primary-nameAnchor"><a class="com_bookingforconnector-item-primary-nameAnchor eectrack" href="<?php echo $route ?>" data-type="Sales Resource" data-id="<?php echo $resource->ResourceId?>" data-index="<?php echo $mrcKey?>" data-itemname="<?php echo $resource->Name; ?>" data-category="<?php echo $resource->MerchantCategoryName; ?>" data-brand="<?php echo $resource->MerchantName; ?>"><?php echo  $resourceName ?></a></div>
				</div>
		...
		<div class="<?php echo COM_BOOKINGFORCONNECTOR_BOOTSTRAP_ROW ?> secondarysection"  style="padding-top: 10px !important;padding-bottom: 10px !important;">
				...
				<div class="<?php echo COM_BOOKINGFORCONNECTOR_BOOTSTRAP_COL ?>2 com_bookingforconnector-item-secondary-section-3 secondarysectionitem">
						<a href="<?php echo $route ?>" class=" com_bookingforconnector-item-secondary-more eectrack" data-id="<?php echo $resource->ResourceId?>" data-type="Sales Resource" data-index="<?php echo $mrcKey?>" data-itemname="<?php echo $resource->Name; ?>" data-category="<?php echo $resource->MerchantCategoryName; ?>" data-brand="<?php echo $resource->MerchantName; ?>"><?php echo JTEXT::_('COM_BOOKINGFORCONNECTOR_MERCHANTS_VIEW_MERCHANTDETAILS_OFFER_DETAILS')?></a>
				</div>
			</div>
