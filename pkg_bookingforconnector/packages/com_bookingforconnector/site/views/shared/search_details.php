...
//	$currentCartConfiguration = BFCHelper::GetCartByExternalUser($tmpUserId, $language, true);
//	$cartEmpty=empty($currentCartConfiguration);
//	$cartConfig = null;
//	if(!$cartEmpty){
//		$cartConfig = json_decode(($currentCartConfiguration->CartConfiguration));
//		if(isset($cartConfig->Resources) && count($cartConfig->Resources)>0){
//			foreach ($cartConfig->Resources as $keyRes=>$resource) {
//				$resetCart = $resource->IsBookable == 0? 1:0;
//				 break;
//			}
//		}
//
...
//	$currUriresource .= "&fromsearch=1";
	...
	$formRoute = JRoute::_($currUriresource);
}else{
	$formRoute = JRoute::_($routeMerchant);
}
...
<br />
<!-- form fields -->
<h4 class="bfi-titleform"><?php echo JText::_('COM_BOOKINGFORCONNECTOR_RESOURCE_VIEW_CALCULATOR_TITLE') ?>
	<div class="bfi-pull-right"><a href="<?php echo $url_cart_page ?>" class="bfi-shopping-cart"><i class="fa fa-shopping-cart "></i> <span class="bfibadge" style="<?php echo (COM_BOOKINGFORCONNECTOR_SHOWBADGE) ?"":"display:none"; ?>"><?php echo ($currentCartsItems>0) ?$currentCartsItems:"";?></span><?php echo JText::_('COM_BOOKINGFORCONNECTOR_ORDERS_VIEW_CART') ?></a></div>
	...
	<div class="bfi-row bfi_resource-calculatorForm-mandatory nopadding">
			<div class="bfi-row nopadding">
				<div class="bfi-col-md-8">
					<div class="bfi-row nopadding">
						<div class="bfi-col-md-6 bfi-col-xs-6" id="calcheckin">      
							...
							<label><?php echo JText::_('COM_BOOKINGFORCONNECTOR_RESOURCE_VIEW_CALCULATOR_CHECKIN') ?></label>
						...
						<div class="bfi-col-md-6 bfi-col-xs-6 <?php echo ($ProductAvailabilityType == 3 || $ProductAvailabilityType == 2)? "bfi-hide " : " "  ?>" id="calcheckout">
							<label><?php echo JText::_('COM_BOOKINGFORCONNECTOR_RESOURCE_VIEW_CALCULATOR_CHECKOUT') ?></label>
							<div class="bfi-datepicker">
							<input type="hidden" name="checkout" value="<?php echo $checkout->format('d/m/Y'); ?>" id="<?php echo $checkoutId; ?>" readonly="readonly"/>
							</div>
							<div class="bfi-duration <?php echo ($ProductAvailabilityType == 3 || $ProductAvailabilityType == 2 || empty($resourceId))? "bfi-hide " : " "  ?>">
								&nbsp;(<span class="calendarnight" id="durationdays"><?php echo $duration ?></span> <span class="calendarnightlabel"><?php echo $ProductAvailabilityType == 1 ? JTEXT::_('MOD_BOOKINGFORSEARCH_NIGHT') : JTEXT::_('MOD_BOOKINGFORSEARCH_DAYS')   ?></span>)
							</div>
						</div>
					</div>
				</div>
				...
				<div class="bfi_resource-calculatorForm-childrenages" style="display:none;">
					<span class="fieldLabel" style="display:inline"><?php echo JTEXT::_('MOD_BOOKINGFORSEARCH_CHILDRENAGE'); ?>:</span>
			...
			<div id="bfishowpersoncalculator" style="display:none;" >
							...
							<div class="bfi-showperson-text-calculator bfi-container">
								...
								<span id="bfi-adult-info-calculator" class="bfi-comma"><span><?php echo $nad ?></span> <?php echo JTEXT::_('MOD_BOOKINGFORSEARCH_ADULTS'); ?></span>
								<span id="bfi-senior-info-calculator" class="bfi-comma"><span><?php echo $nse ?></span> <?php echo JTEXT::_('MOD_BOOKINGFORSEARCH_SENIORES'); ?></span>
								<span id="bfi-child-info-calculator" class="bfi-comma"><span><?php echo $nch ?></span> <?php echo JTEXT::_('MOD_BOOKINGFORSEARCH_CHILDREN'); ?></span>
							</div>
			</div>
...
<div class="bfi-hide">
		<div class="bfi-timeperiod-change" id="bfimodaltimeperiod">
				...
				<div class="bfi-simplerow bfi-text-center">
					<a id="bfi-timeperiod-select" class="bfi-btn" onclick="bfi_selecttimeperiod(this)" data-rateplanid="0" data-resid="0"><?php echo JTEXT::_('COM_BOOKINGFORCONNECTOR_RESOURCE_VIEW_SELECT') ?></a>
				</div>	
		</div>
</div><!-- /.modal -->
...
<div class="bfi-hide">
		<div class="bfi-timeslot-change" id="bfimodaltimeslot">
				...
				<div class="bfi-simplerow bfi-text-center">
					<a id="bfi-timeslot-select" class="bfi-btn" onclick="bfi_selecttimeslot(this)" data-rateplanid="0" data-resid="0" data-sourceid="0"><?php echo JTEXT::_('COM_BOOKINGFORCONNECTOR_RESOURCE_VIEW_SELECT') ?></a>
				</div>	
		</div>
</div><!-- /.modal -->
<?php 
		...
		<table class="bfi-table bfi-table-bordered bfi-table-resources bfi-table-resources-sticked" style="margin-top: 20px;">
			<thead>
				<tr>
					...
					<th><div><?php echo JTEXT::_('COM_BOOKINGFORCONNECTOR_RESOURCE_VIEW_CONFIRMRESERVATION') ?></div></th>
				</tr>
			</thead>
				...
				<td rowspan="400">
							<div class="bfi-book-now">
								...
								<div id="btnBookNow" class="bfi-btn bfi-btn-book-now" data-formroute="<?php// echo $formRouteBook ?>" onclick="ChangeVariation(this);">
									<?php echo JTEXT::_('COM_BOOKINGFORCONNECTOR_MERCHANTS_VIEW_MERCHANTDETAILS_RESOURCE_BUTTON_HTTPS') ?>
								</div>
								<div class="bfi-btn bfi-alternative bfi-request-now" onclick="ChangeVariation(this);">
									<?php echo JTEXT::_('COM_BOOKINGFORCONNECTOR_MERCHANTS_VIEW_MERCHANTDETAILS_RESOURCE_BUTTON_REQ') ?>
								</div>
...
<?php } ?>
			...
			<?php if (!empty($allResourceId)) { ?>
				<tr><td colspan="5" class="bfi-otherresults-box"><div class="bfi-otherresults"><?php echo sprintf(JTEXT::_('COM_BOOKINGFORCONNECTOR_RESOURCE_VIEW_OTHERCHOISE') , $totalResCount) ?></div> <?php echo JTEXT::_('COM_BOOKINGFORCONNECTOR_RESOURCE_VIEW_OTHERCHOISE_DESCR') ?></td></tr>
			<?php } ?>
...
/*-----------scelta date e ore--------------------*/	
									...
									{
										?>
									  <span class="bfi-availability-low"><?php echo sprintf(JTEXT::_('COM_BOOKINGFORCONNECTOR_MERCHANTS_VIEW_MERCHANTDETAILS_RESOURCE_LESSAVAIL') ,$res->Availability) ?></span>
									<?php 
									}
									...
									?>
										...
										>
											...
											<div class="bfi-row">
												...
												<div class="bfi-col-md-9 bfi-text-right"><span class="bfi-total-duration"><?php echo $timeDurationview ?></span> <?php echo JTEXT::_('COM_BOOKINGFORCONNECTOR_RESOURCE_VIEW_HOURS') ?>
												</div>	
											</div>	
										</div>
								<?php
...
if(!empty( $currRateplan->RatePlan->MerchantBookingTypesString )){
	...
	if(count($currMerchantBookingType)>0){
		if($currMerchantBookingType[0]->PayOnArrival){
			$prepayment = JTEXT::_('COM_BOOKINGFORCONNECTOR_RESOURCE_PREPAYMENT') ;
			$prepaymentHelp = JTEXT::_('COM_BOOKINGFORCONNECTOR_RESOURCE_PREPAYMENT_HELP') ;
		}
		if($currMerchantBookingType[0]->AcquireCreditCardData){
			$prepayment = "";
			if($currMerchantBookingType[0]->DepositRelativeValue=="100%"){
				$prepaymentHelp = JTEXT::_('COM_BOOKINGFORCONNECTOR_RESOURCE_PREPAYMENT_HELP1') ;
			}else if(strpos($currMerchantBookingType[0]->DepositRelativeValue, '%') !== false  ) {
				$prepaymentHelp = sprintf(JTEXT::_('COM_BOOKINGFORCONNECTOR_RESOURCE_PREPAYMENT_HELP2') ,$currMerchantBookingType[0]->DepositRelativeValue);
			}else{
				$prepaymentHelp = sprintf(JTEXT::_('COM_BOOKINGFORCONNECTOR_RESOURCE_PREPAYMENT_HELP3') ,$currMerchantBookingType[0]->DepositRelativeValue);
			}
		}
	}
}
...
?>
		...
		if (count($allResourceId) > 1 && (($resId == $resourceId && $resCount > 1) || ($resId == $resourceId && $resCount == 0))): ?>
				<tr><td colspan="5" class="bfi-otherresults-box"><div class="bfi-otherresults"><?php echo sprintf(JTEXT::_('COM_BOOKINGFORCONNECTOR_RESOURCE_VIEW_OTHERCHOISE') , (count($allResourceId)-1)) ?></div> <?php echo JTEXT::_('COM_BOOKINGFORCONNECTOR_RESOURCE_VIEW_OTHERCHOISE_DESCR') ?></td></tr>
		<?php endif; ?>
	<?php 
		$resCount++;
 } 
...
<?php if(count($allResourceId)>0){ ?>
	...
	<br /><?php  BFCHelper::bfi_get_template('shared/menu_small_booking.php');  ?>
...
<table class="bfi-table bfi-table-bordered bfi-table-resources bfi-table-selectableprice bfi-table-selectableprice-container bfi-table-resources-sticked" style="margin-top: 20px;">
			<thead>
				<tr>
					<th><?php echo JTEXT::_('COM_BOOKINGFORCONNECTOR_RESOURCE_ADDMORE') ?></th>
					<th><div><?php echo JTEXT::_('COM_BOOKINGFORCONNECTOR_RESOURCE_VIEW_CONFIRMRESERVATION') ?></div></th>
				</tr>
			</thead>
		<tr>
...
//
//$currCheckInString = JTEXT::_('COM_BOOKINGFORCONNECTOR_RESOURCE_SELECTPERIOD') ;
//$currCheckOutString = "";
//$currCheckInHour = "";
//$currCheckOutHour = "";
//$currDiffString = "-";
...
//					printChangedDateBooking(date, jQuery(this)); 
			...
			, beforeShowDay: function (date) {
				return closedBooking(date, 0, daysToEnable); 
				}
			...
			, buttonText: "<div class='checkinBooking'><span class='bfi-weekdayname'><?php echo $checkin->format("l");?> </span><?php echo $checkin->format("d") ;?> <?php echo $checkin->format("M");?><span class='bfi-year'> <?php echo $checkin->format("Y"); ?></span></div>"
		...
		calculator_checkout = function() { $("#<?php echo $checkoutId; ?>").datepicker({
			...
			, buttonText: "<div class='checkoutBooking'><span class='bfi-weekdayname'><?php echo $checkout->format("l");?> </span><?php echo $checkout->format("d") ;?> <?php echo $checkout->format("M");?><span class='bfi-year'> <?php echo $checkout->format("Y"); ?></span></div>"
		})};
...
<?php if(!empty($variationPlanMinDuration)) { ?>
	offsetDate.setDate(offsetDate.getDate() + <?php echo $variationPlanMinDuration //night ?>);
<?php } ?>
...
function getCheckinAjaxDate(callback) {
	...
	var options = { 
	    url:        bfi_variable.bfi_urlCheck + ((bfi_variable.bfi_urlCheck.indexOf('?') > -1)? "&" :"?") + 'task=listDateCheckin&resourceId=' + unitId + '&simple=1', 
	    dataType: 'text',
		success: function(data) { 
            daysToEnable = data.split(',');
			jQuery("#calcheckin").unblock();
			for(var i=0; i<daysToEnable.length; i++) { daysToEnable[i] = +daysToEnable[i]; } 
...
function getAjaxOptions(callback) {
	...
	formUrl = formUrl + ((formUrl.indexOf('?') > -1)? "&" :"?") + 'format=calc&tmpl=component'
	var options = { 
	    target:     '#calculator',
	    replaceTarget: true, 
	    url:        formUrl, 
...
<?php if($loadScriptTimePeriod) { 
    ...
    <script>
        ...
        var strbuttonTextTimePeriod = "<?php echo $checkin->format("D") ?> <?php echo $checkin->format("d") ?> <?php echo $checkin->format("M").' '.$checkin->format("Y") ?>";
...
//			jQuery(".bfi-timeperiod-change").dialog('destroy');
			dialogTimeperiod = jQuery("#bfimodaltimeperiod").dialog({
				title: "<?php echo JTEXT::_('COM_BOOKINGFORCONNECTOR_RESOURCE_VIEW_DIALOGTIME_TITLE') ?>",
				autoOpen: false,
				modal: true,
				width: 'auto',
				maxWidth: "300px",
...
//	}
    ...
    <script>
        //TimeSlot
        var strbuttonTextTimeSlot = "<?php echo $checkin->format("D") ?> <?php echo $checkin->format("d") ?> <?php echo $checkin->format("M").' '.$checkin->format("Y") ?>";
